...
import { fetchCSVData } from '../services/tableRequest';
...
const CodeController = ({ query, setQuery , setTableData, setRecentQuery }) => {
    const { setBackdropOpen, triggerSnackbar } = useContext(Context);
        ...
        if(query !== ""){
            ...
            .then((res)=>{
                ...
                if(res.length){
                    ...
                    triggerSnackbar(3000, "No data found","error" ,{ vertical: 'bottom', horizontal: 'right' });
                }
    ...
    return (
        <div className="editor-controller">
            <Button onClick={handleQuerySubmit} title="Execute" icon={<SendIcon sx={{fontSize:15}}/>}/><div className='button-space'></div>
            <Button onClick={handleQueryClear} title="Clear Editor" icon={<DeleteIcon sx={{fontSize:15}}/>}/>
        </div>
    ) 
}
